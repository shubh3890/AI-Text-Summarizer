...
# HF retired api-inference.huggingface.co in late 2025; the replacement is the
# "router" endpoint, using the hf-inference provider for community/custom models.
...
def summarize_dialogue(dialogue: str) -> str:
    ...
    if response.status_code == 404:
        raise HTTPException(
            status_code=502,
            detail=(
                "Hugging Face's hf-inference provider doesn't currently serve this "
                "model (404). This can happen for custom fine-tuned models that "
                "aren't on the supported list. Consider deploying the model as a "
                "Hugging Face Space instead (Gradio/FastAPI), or using a dedicated "
                "Inference Endpoint."
            )
        )
